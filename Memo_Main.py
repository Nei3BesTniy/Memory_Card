...
from Memo_Edit_Layout import txt_Question, txt_Answer, txt_Wrong1, \
    txt_Wrong2, txt_Wrong3
from App import *
...
time_unit = 60000  # столько длится одна единица времени из тех, на которые нужно засыпать

######################################          Глобальные переменные:      #############################################
questions_listmodel = QuestionListModel()  # список вопросов
frm_edit = QuestionEdit(0, txt_Question, txt_Answer, txt_Wrong1, txt_Wrong2, txt_Wrong3)  # Запоминаем, что в форме редактирования вопрос с чем связано
radio_list = [rbtn_1, rbtn_2, rbtn_3, rbtn_4]  # список виджетов, которые нужно перемешать (для случайного размещения ответов)
frm_card = 0  # тут будут связываться вопросы с формой теста
timer = QTimer()  # таймер для возможности "заснуть" на определенное время и проснутся
win_card = QWidget()  # окно карты
...
def set_card():
    ''' задаёт, начальный взгляд окна карты'''
...
def sleep_card():
    ''' окошко пряется на время, указанное в таймере'''
    win_card.hide()
    timer.setInterval(time_unit * box_Minutes.value())
    timer.start()

def show_card():
    ''' показует окно (по таймеру), таймер останавливаеться'''
    win_card.show()
    timer.stop()

def show_random():
    ''' показать случайный вопрос '''
    global frm_card  # свойство окна - властивість вікна - текущая форма с данными формы
    # получаем случайные данные, но случайно же распределяем варианты ответов по радиокнопкам:
    frm_card = random_AnswerCheck(questions_listmodel, lb_Question, radio_list, lb_Correct, lb_Result)
    # мы будем запускать функцию, когда окно уже открыто. Тогда показуем:
    frm_card.show()  # загрузить нужные данные в соответствующие виджеты
    show_question()  # показать в форме панель вопросов
...
def click_OK():
    ...
    if btn_OK.text() != 'Следующий вопрос':
        ...
        # надпис на кнопке равняется 'Следующий', вот и создаем новый случайный вопрос:
        show_random()

def back_to_menu():
    ''' возвращение из теста в окно редактора '''
    win_card.hide()
    win_main.showNormal()

######################################     Функции для редактирования вопросов:    ######################################
def set_main():
    ''' задает, как выглядит основное окно'''
...
def edit_question(index):
    ''' загружает в форму вопросы и ответы, соответствующие переданной строке '''
    #  index - это объект класса QModelIndex, смот. требуемые методы ниже
    if index.isValid():
        i = index.row()
...
def add_form():
    ...
    questions_listmodel.insertRows()  # новые вопросы появляются в данних и автоматически в списке на екране
    last = questions_listmodel.rowCount(0) - 1  # новый вопрос - последнее в списке. Найдём его позицию.
